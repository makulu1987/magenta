...
status_t IommuImpl::Create(const mx_iommu_desc_intel_t* desc, uint32_t desc_len,
                           mxtl::RefPtr<Iommu>* out) {
    status_t status = ValidateIommuDesc(desc, desc_len);
    if (status != MX_OK) {
        return status;
    }
    ...
    AllocChecker ac;
    auto instance = mxtl::AdoptRef<IommuImpl>(new (&ac) IommuImpl(vaddr));
    if (!ac.check()) {
...
status_t IommuImpl::ValidateIommuDesc(const mx_iommu_desc_intel_t* desc, uint32_t desc_len) {
    // Validate the size
    if (desc_len < sizeof(*desc)) {
        return MX_ERR_INVALID_ARGS;
    }
    static_assert(sizeof(desc->num_scopes) == 1, "if this changes, need to check for overflow");
    const size_t actual_size = sizeof(*desc) + desc->num_scopes * sizeof(desc->scopes[0]);
    if (desc_len != actual_size) {
        return MX_ERR_INVALID_ARGS;
    }

    // Validate scopes
    if (desc->num_scopes == 0 && !desc->whole_segment) {
        return MX_ERR_INVALID_ARGS;
    }
    for (size_t i = 0; i < desc->num_scopes; ++i) {
        if (desc->scopes[i].num_hops == 0) {
            return MX_ERR_INVALID_ARGS;
        }
        if (desc->scopes[i].num_hops > countof(desc->scopes[0].dev_func)) {
            return MX_ERR_INVALID_ARGS;
        }
    }
    return MX_OK;
